...
void
NFA::connect_edge(Input symbol, Edge::Node& state, size_t location)
{
...
void
NFA::connect_branch(Branch& to, Branch& from)
{
...
NFA::connect_NFA(NFA& nfa)
{
    ...
    std::ranges::for_each(
      nfa.edges, [=, this, &front, &back](NFA::Branch& branch) {
          std::ranges::for_each(
            branch.begin(), branch.end(), [=, this, &front, &back](auto& edge) {
                edge.nodes.first.get()->type = edge.nodes.second.get()->type =
                  State::Type::normal;
